"""
reasoners.py — The five-agent Investment Committee for Equinox.
...
async def plan_research(query: str) -> DualResearchReport:
    ...
    # Step 2: Sequential dispatch → Analyst (bull) then Contrarian (bear) to avoid 5 RPM rate limits
    max_retries = 2
    ...
    for attempt in range(max_retries + 1):
        ...
        if analyst_finding is None:
            app.note(f"[manager] Waiting 12s for rate limit before Analyst...", tags=["manager", plan.ticker])
            await asyncio.sleep(12)
            analyst_finding = await conduct_research(plan)

        if risk_assessment is None:
            app.note(f"[manager] Waiting 12s for rate limit before Contrarian...", tags=["manager", plan.ticker])
            await asyncio.sleep(12)
        ...
        if analyst_finding.data_quality == "low" and attempt < max_retries:
            ...
            app.note(f"[manager] Waiting 12s for rate limit before plan refinement...", tags=["manager", plan.ticker])
            await asyncio.sleep(12)
            plan = await app.ai(
                system=(
    ...
    # Step 3: Dual editors in sequence (short-term then long-term)
    app.note(f"[manager] Waiting 12s for rate limit before Editor Short...", tags=["manager", plan.ticker])
    await asyncio.sleep(12)
    short_report = await run_editor_short()

    app.note(f"[manager] Waiting 12s for rate limit before Editor Long...", tags=["manager", plan.ticker])
    await asyncio.sleep(12)
